...
BEGIN
    ...
    -- Get the status of the bookcopy
SELECT status INTO book_status FROM bookcopies WHERE bookcopy_id = NEW.bookcopy_id;

-- Check if the bookcopy status is 'LOST' and raise an error if true
IF book_status = 'LOST' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Cannot make a reservation for a lost book copy';
END IF;
END
